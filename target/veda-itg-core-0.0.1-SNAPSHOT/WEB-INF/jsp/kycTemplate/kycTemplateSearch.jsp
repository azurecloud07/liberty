							...
							<div class="bt-form__wrapper">
								<div class="form-group uk-width-1-1">
									<div class="bt-flabels__wrapper">
										...
										<form:select path="countryOfResidenceCode" class="form-control select2me" data-parsley-required="false" placeholder="${countryOfResidenceLabel}">
											<form:option value=""></form:option>
							...
							<div class="bt-form__wrapper">
								<div class="form-group uk-width-1-1">
									<div class="bt-flabels__wrapper">
										...
										<form:select path="riskCategoryCode" class="form-control select2me" data-parsley-required="false" placeholder="${riskCategoryLabel}">
											<form:option value=""></form:option>
...
<script>
				...
				$("#templateJqxgrid").jqxGrid(
						{
							...
							columns: [
							  ...
							  { text: 'Template Code', datafield: 'templateCode', filtertype: 'input', width: '12%', align: 'left'},
							  { text: 'Jurisdiction', datafield: 'jurisdiction', filtertype: 'input', width: '12%', align: 'left'},
							  { text: 'Counter Party', datafield: 'counterparty', filtertype: 'input', width: '8%', align: 'left'},
							  { text: 'Risk Category', datafield: 'riskCategory', filtertype: 'input', width: '8%', align: 'left'},
							  { text: 'Roles', datafield: 'roleName', filtertype: 'input', width: '15%', align: 'left'},
							  { text: 'Approval Status', datafield: 'approvalStatus', filtertype: 'input', width: '12%', align: 'left'},
							  { text: 'Status', datafield: 'status', filtertype: 'input', width: '8%', align: 'left' , enabletooltips:false},
				...
				$("#templateJqxgrid").jqxGrid(
						{
							...
							columns: [
							  ...
							  { text: 'Template Code', datafield: 'templateCode', filtertype: 'input', width: '12%', align: 'left'},
							  { text: 'Jurisdiction', datafield: 'jurisdiction', filtertype: 'input', width: '12%', align: 'left'},
							  { text: 'Counter Party', datafield: 'counterparty', filtertype: 'input', width: '13%', align: 'left'},
							  { text: 'Risk Category', datafield: 'riskCategory', filtertype: 'input', width: '13%', align: 'left'},
							  { text: 'Approval Status', datafield: 'approvalStatus', filtertype: 'input', width: '12%', align: 'left'},
							  { text: 'Status', datafield: 'status', filtertype: 'input', width: '8%', align: 'left' , enabletooltips:false},
							  { text: 'Action', datafield: 'action', filtertype: 'input', width: '15%', align: 'left' , enabletooltips:false}
							],
	...
	$('#searchButton').click(function() {
		_paq.push(['trackEvent', 'KYCTemplate', 'SearchFilter','Filtering template']);
		$('#frm').attr("action", "${contextPath}/kyc/template/searchfilter");
	...
	$('#addNewButton').click(function() {
		_paq.push(['trackEvent', 'KYCTemplate', 'Add','Adding template']);
		location.href = "${contextPath}/kyc/template/add";
		buffer();
	});
	
	$('#clearButton').click(function() {
		_paq.push(['trackEvent', 'KYCTemplate', 'Clear','Clear filter']);
		location.href = "${contextPath}/kyc/template/search";
		buffer();
	});
	
	function getViewPage(contextPath, id) {
		_paq.push(['trackEvent', 'KYCTemplate', 'View','Viewing template']);
	...
	function getEditPage(contextPath, id) {
		_paq.push(['trackEvent', 'KYCTemplate', 'Edit','Editing template']);
	...
	function getAuditLogPage(contextPath, id) {
		_paq.push(['trackEvent', 'KYCTemplate', 'Audit','Auditing template']);
		$('#frm').attr("action", contextPath + "/kyc/template/audit");
		$('#frm').attr("method", "post");
	...
	function cloneTemplate(contextPath, id) {
		_paq.push(['trackEvent', 'KYCTemplate', 'Clone','Cloning template']);
		var newForm = jQuery('<form>', {
	        'action': '${contextPath}/kyc/template/clone',
	        'method':'POST'
	    }).append(jQuery('<input>', {
	        'name': 'id',
	        'value': id,
	...
	function setActivateOrDeactivate(contextPath, id, status) {
		...
		bootbox.confirm({
	        ...
	        callback: function (result) {
	        	if(result){
	        		if(status == '1')
	        			_paq.push(['trackEvent', 'KYCTemplate', 'Activate', 'Activating template']);
	        		else if(status == '3')
	        			_paq.push(['trackEvent', 'KYCTemplate', 'Deactivate', 'Deactivating template']);
		        	
	        		$('#frm').attr("action", contextPath + "/kyc/template/process");
	        		$('#frm').attr("method", "post");
