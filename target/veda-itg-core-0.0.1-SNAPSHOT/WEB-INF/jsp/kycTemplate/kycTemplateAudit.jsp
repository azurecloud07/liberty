	...
	<div class="page-content">
		<div class="page-bar search-page-bar">
			<ul class="page-breadcrumb pull-right">
                ...
                <li><a href="/digiKYC/kyc/template/search/"> KYC Documents Template<i class="fa fa-angle-right"></i></a></li>
				<li>Audit</li>
			</ul>
		...
		<form:form action="${contextPath}/kyc/template/auditfilter" method="POST" id="frm" role="form" class="horizontal-form uk-form bt-flabels js-flabels" data-parsley-validate="true" data-parsley-errors-messages-disabled="true">
...
<script>
		...
		if($("#templateAuditJson").val() != null && $("#templateAuditJson").val() != undefined && $("#templateAuditJson").val() != "") {
			...
			var templateDataSource =
			{
				...
				datafields: [
							...
							{ name: 'countryOfResidenceName', type: 'string'},
							{ name: 'typeOfCounterPartyName', type: 'string'},
							{ name: 'riskCategoryName', type: 'string'},
			...
			$("#templateJqxgrid").jqxGrid(
			{
				...
				columns: [
					  ...
					  { text: 'Jurisdiction', datafield: 'countryOfResidenceName', filtertype: 'input', width: '9%', align: 'left' , cellclassname: cellRendererWithBackgroundColor},
					  { text: 'Counter Party', datafield: 'typeOfCounterPartyName', filtertype: 'input', width: '9%', align: 'left' , cellclassname: cellRendererWithBackgroundColor},
					  { text: 'Risk Category', datafield: 'riskCategoryName', filtertype: 'input', width: '9%', align: 'left' , cellclassname: cellRendererWithBackgroundColor},
	...
	$('#searchButton').click(function() {
		...
		_paq.push(['trackEvent', 'KYCTemplate', 'SearchFilterAudit','Filtering audit template']);
		$('#frm').attr("action", "${contextPath}/kyc/template/auditfilter");
	...
	$('#cancelButton').click(function() {
		_paq.push(['trackEvent', 'KYCTemplate', 'Cancel','Cancel filtering audit template']);
		location.href = "${contextPath}/kyc/template/search";
		buffer();
	});
	
	function getAuditViewPage(id) {
		_paq.push(['trackEvent', 'KYCTemplate', 'AuditView','Viewing of audit record']);
		window.open("${contextPath}/kyc/template/audit/view/" + id, "vedaNewWindow", "toolbar=no, scrollbars=yes, resizable=no, top=0, left=0, width=1200, height=800");
	}
	...
	$('#compareToButton').click(function() {
		...
		if(templateIdArray.length > 1){
			$('#compare_frm').attr("action", "${contextPath}/kyc/template/compareTo");
			$('#compare_frm').attr("method", "post");
			$('#compare_frm').append('<input type="hidden" name="templateIds" value="' +templateIdArray+ '" />');
			$('#compare_frm').submit();
		}else{
			bootbox.confirm({
