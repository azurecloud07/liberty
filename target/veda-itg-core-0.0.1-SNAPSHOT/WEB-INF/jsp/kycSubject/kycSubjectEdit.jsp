...
<div class="page-content-wrapper">
	<div class="page-content">
			...
			<div class="col-md-12">
				<div class="portlet-body form">
					<div class="pull-right">
						<button type="button" class="btn btn-link" data-toggle="modal" data-target="#remarkHistoryModalId" >View Remarks</button>
						<button type="button" id="updateButton"
							class="btn btn-sm grey-gallery">
							<em class="fa fa-check-circle"></em> ${updateLabel}
						</button>
		...
		<form:form action="${contextPath}/kyc/subject/edit" method="POST" id="frm" role="form" class="horizontal-form uk-form bt-flabels js-flabels" modelAttribute="kycSubjectBO" data-parsley-validate="true" data-parsley-errors-messages-disabled="false" enctype="multipart/form-data">
			<form:hidden path="id" />
					...
					<div class="bt-form__wrapper">
						<div class="form-group uk-width-1-1">
							<div class="bt-flabels__wrapper">
								...
								<div class="field-background">
									<form:select id="countryOfNationalityCode"
										path="countryOfNationalityCode"
							...
							<div class="tab-pane fade in" id="tab_${proofStatus.index}"> 
								...
								<c:if test="${proofObject.documentList != null && proofObject.documentList.size() > 0}">
									<c:forEach items="${proofObject.documentList}" var="documentObject" varStatus="docStatus">
										<div class="row">
											<div class="col-md-4">
												<div class="form-group">
													<input type="hidden" id="isDocSelected${proofStatus.index}_${docStatus.index}_hidden" name="proofList[${proofStatus.index}].documentList[${docStatus.index}].isDocumentSelected" value="${documentObject.isDocumentSelected}" />
													<c:choose>
														<c:when test="${documentObject.isDocumentSelected}">
															<input type="checkbox" checked="checked" name="proofList[${proofStatus.index}].documentList[${docStatus.index}].documentId" id="isDocSelected${proofStatus.index}_${docStatus.index}" value="${documentObject.documentId}" onclick="javascript:displaydocs(this,'${proofStatus.index}','${docStatus.index}','${documentObject.documentId}','${documentObject.documentAttributeList.size()}');">
														</c:when>
														<c:otherwise>
											...
											<c:choose>
												<c:when test="${documentObject.isDocumentSelected}">
													<div class="col-md-8 document${proofStatus.index}_${docStatus.index}" >
												</c:when>
												<c:otherwise>
												...
												<div class="radio-list">
													...
													<label class="radio-inline">
														<c:choose>
															<c:when test="${documentObject.documentReceived == 1}">
																<c:choose>
													...
													<label class="radio-inline">
														<c:choose>
															<c:when test="${documentObject.documentReceived == 0}">
																<c:choose>
																	<c:when test="${documentObject.documentAttributeList !=null && documentObject.documentAttributeList.size() > 0}">
																		<input type="radio" checked="checked" name="proofList[${proofStatus.index}].documentList[${docStatus.index}].documentReceived" id="isDocumentReceivedNo${proofStatus.index}_${docStatus.index}" value="0" onclick="showdocFields(this.value,'${proofStatus.index}','${docStatus.index}','${documentObject.documentAttributeList.size()}')"><span>&nbsp;No&nbsp;&nbsp;</span>
																	</c:when>
																	<c:otherwise>
																		<input type="radio" checked="checked" name="proofList[${proofStatus.index}].documentList[${docStatus.index}].documentReceived" id="isDocumentReceivedNo${proofStatus.index}_${docStatus.index}" value="0" onclick="showdocFields(this.value,'${proofStatus.index}','${docStatus.index}')"><span>&nbsp;No&nbsp;&nbsp;</span>
																	</c:otherwise>
																</c:choose>
															</c:when>
															<c:otherwise>
													...
													<label class="radio-inline">
														<c:choose>
															<c:when test="${documentObject.documentReceived == 2}">
																<c:choose>
																	<c:when test="${documentObject.documentAttributeList !=null && documentObject.documentAttributeList.size() > 0}">
																		<input type="radio" checked="checked" name="proofList[${proofStatus.index}].documentList[${docStatus.index}].documentReceived" id="isDocumentReceivedWaiver${proofStatus.index}_${docStatus.index}" value="2" onclick="showdocFields(this.value,'${proofStatus.index}','${docStatus.index}','${documentObject.documentAttributeList.size()}')"><span>&nbsp;Waiver&nbsp;&nbsp;</span>
																	</c:when>
																	<c:otherwise>
																		<input type="radio" checked="checked" name="proofList[${proofStatus.index}].documentList[${docStatus.index}].documentReceived" id="isDocumentReceivedWaiver${proofStatus.index}_${docStatus.index}" value="2" onclick="showdocFields(this.value,'${proofStatus.index}','${docStatus.index}')"><span>&nbsp;Waiver&nbsp;&nbsp;</span>
																	</c:otherwise>
																</c:choose>
										...
										<c:choose>
											<c:when test="${documentObject.documentReceived == 0}">
												<div class="row docFieldDetailsNo${proofStatus.index}_${docStatus.index}" >
											</c:when>
											<c:otherwise>
										...
										<c:choose>
											<c:when test="${documentObject.documentReceived == 2}">
												<div class="row docFieldDetailsWaiver${proofStatus.index}_${docStatus.index}" >
											</c:when>
											<c:otherwise>
										...
										<c:choose>
											<c:when test="${documentObject.documentReceived == 1}">
												<div class="row docFieldDetailsYes${proofStatus.index}_${docStatus.index}" >
											</c:when>
											<c:otherwise>
											...
											<div class="col-md-4">
												<div class="form-group uk-width-1-1">
													<div class="bt-flabels__wrapper">
														...
														<form:hidden id="fileId_${proofStatus.index}_${docStatus.index}" path="proofList[${proofStatus.index}].documentList[${docStatus.index}].fileId" value=""/>
													</div>
												</div>
											</div>
											<div class="col-md-2">
...
<script>
	...
	jQuery(document).ready(function() {
		...
		$('.documentDetails').find('input[type="checkbox"]:checked').each( function() {
	...
	$('#updateButton').click(function() {
		...
		if(response === true){
			...
			$('#frm').attr("action", "${contextPath}/kyc/subject/edit");
			bootbox.confirm({
	...
	$("#typeOfCounterpartyCode").change(function() {
		...
		if(typeOfCounterpartyCode!=null && typeOfCounterpartyCode != ""){
			...
			$('#frm').parsley().destroy();
			$('#frm').attr("method", "post");
			$('#frm').attr("action", "${contextPath}/kyc/subject/addPage");
			$('#frm').submit();
			buffer();
		}
	});
	...
	function displaydocs(value, proofIndex, documentIndex, documentId, attributeCount) {
		...
		if (value.checked) {
			...
			if(jQuery.inArray(documentId, documentIds) !== -1) {
